# Distributing MCP Servers With OCI To Power Agent Skills

**Bobby House** — Sr. Software Engineer, Docker
MCP Dev Summit 2026

## References

### MCP Servers

#### Project Gutenberg MCP
- GitHub: https://github.com/bobbyhouse/project-gutenberg-mcp
- Docker Hub: https://hub.docker.com/r/roberthouse224/project-gutenberg-mcp
- Registry: `io.github.bobbyhouse.project-gutenberg-mcp`

#### Append Log MCP
- GitHub: https://github.com/bobbyhouse/append-log-mcp
- Docker Hub: https://hub.docker.com/r/roberthouse224/append-log-mcp
- Registry: `io.github.bobbyhouse.append-log-mcp`

### Claude Code Skills

#### Client Plugins
- GitHub: https://github.com/bobbyhouse/client-plugins
- Contains plugins for skill-builder and demos of skills built with skill-builder

### Docker

- Docker MCP Gateway: https://github.com/docker/mcp-gateway/
- Docker MCP Catalog & Toolkit – Profiles: https://docs.docker.com/ai/mcp-catalog-and-toolkit/profiles/
